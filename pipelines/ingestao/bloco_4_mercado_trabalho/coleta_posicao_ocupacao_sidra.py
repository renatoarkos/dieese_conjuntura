"""Coleta bruta da população ocupada por posição na ocupação e categoria do emprego
...
Fonte confirmada em: docs/04-fontes/ibge-sidra.md (Tabela SIDRA 4097) — Lote 02,
2026-09-22.
...
dimensão da tabela:
    ...
    https://apisidra.ibge.gov.br/values/t/{tabela}/n1/{territorio}/v/{variavel}/p/{periodo}/c11913/{classificacao}

  - `t/4097`        → número da tabela no SIDRA (cada tabela do IBGE tem um ID
                      fixo; 4097 é "População e posição na ocupação", um dos
                      resultados da PNAD Contínua trimestral). Para achar o
                      número de outra tabela, procure no site sidra.ibge.gov.br
                      e copie da URL da tabela.
  ...
  - `c11913/all`    → um filtro de classificação específico desta tabela (aqui:
                      `c11913` é "Posição na ocupação e categoria do emprego" —
                      empregado do setor privado com/sem carteira, trabalhador
                      doméstico, empregado do setor público, empregador, conta
                      própria, trabalhador familiar auxiliar etc.; "all" traz
                      todas as categorias). Cada tabela do SIDRA tem suas
                      próprias classificações — o número do código muda de
                      tabela para tabela.
...
O QUE A TABELA 4097 MEDE: quantas pessoas de 14 anos ou mais estão ocupadas,
divididas por "posição na ocupação e categoria do emprego" — ou seja, o tipo
de vínculo que a pessoa tem com o trabalho (empregada com carteira assinada,
sem carteira, no setor público, trabalhadora doméstica, empregadora, conta
própria, trabalhadora familiar auxiliar, entre outras). É a fonte para
acompanhar, por exemplo, o peso da informalidade no total de ocupados.
...
proporções, juntar categorias ou qualquer outra conta é trabalho da camada
...
# Tabela 4097 do SIDRA: "População e posição na ocupação" — PNAD Contínua trimestral.
