"""Coleta bruta da taxa de participação na força de trabalho (PNAD Contínua
trimestral) via API pública do SIDRA/IBGE.

Piloto técnico controlado — ver docs/08-decisoes-adr/0002-expansao-piloto-por-blocos.md
Fonte confirmada em: docs/04-fontes/ibge-sidra.md (Tabela SIDRA 6461) — Lote 02,
2026-09-22.
...
dimensão da tabela:
    ...
    https://apisidra.ibge.gov.br/values/t/{tabela}/n1/{territorio}/v/{variavel}/p/{periodo}

  - `t/6461`   → número da tabela no SIDRA (cada tabela do IBGE tem um ID
                 fixo; 6461 é "Taxa de participação na força de trabalho", um
                 dos resultados da PNAD Contínua trimestral). Para achar o
                 número de outra tabela, procure no site sidra.ibge.gov.br e
...
Diferente de outras tabelas do SIDRA (como PIB ou desocupação), esta consulta
não tem nenhum segmento de classificação (`c.../...`) ao final da URL —
porque a Tabela 6461 não tem recorte adicional por sexo, idade ou outra
dimensão: traz só a taxa de participação total.
...
O QUE A TABELA 6461 MEDE: a taxa de participação na força de trabalho — ou
seja, a proporção de pessoas de 14 anos ou mais que estão na força de
trabalho (ocupadas ou desocupadas procurando emprego) em relação ao total da
população nessa faixa etária, na semana de referência da pesquisa. É um
indicador complementar à taxa de desocupação: mede quantas pessoas estão
efetivamente participando do mercado de trabalho (empregadas ou buscando
emprego), e não só quantas dessas estão sem trabalho.
...
# Tabela 6461 do SIDRA: "Taxa de participação na força de trabalho" — PNAD
# Contínua trimestral. Sem classificação adicional (tabela não tem recorte
# de sexo/idade).
