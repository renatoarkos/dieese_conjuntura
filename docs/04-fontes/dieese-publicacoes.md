...
| ⚠️ Divergência entre as duas rodadas sobre o formato do PDF | A 1ª rodada (edição 65) classificou o PDF como "majoritariamente imagem/binário, sem texto extraível". Na 2ª rodada, o agente leu a edição 67 e conseguiu citar texto e números diretamente. **Isso não confirma, por si só, que existe uma camada de texto extraível por script** (`pdftotext`/regex, como no piloto da Cesta Básica) — pode ser leitura visual/multimodal do PDF pela ferramenta do agente, que não é o mesmo que uma extração automatizável em pipeline. **Teste técnico direto pendente** antes de reclassificar de E para B/C: baixar a edição mais recente e testar extração de texto real (`pdftotext`, `pypdf` ou equivalente) sobre os slides "Reajustes salariais" e "Pisos salariais". |
...
| Periodicidade | Mensal. |
| Histórico | Desde pelo menos 2021 (edição nº 06) até abr/2026 (edição nº 67, lida diretamente) / mai/2026 (edição nº 68, mencionada em buscas, não baixada). |
| **Classificação de automação** | **E → C (fonte pública mensal confirmada e lida diretamente; automação por extração de texto de PDF pendente de teste técnico real)**. Se o teste `pdftotext`/regex confirmar camada de texto, sobe para **B — download estruturado** (mesmo padrão da Cesta Básica). Não precisa de scraping do Mediador nem de parceria nova — o boletim do próprio DIEESE já resolve ambiguidades (instrumentos multissetoriais, reajustes escalonados) melhor do que uma reconstrução externa. |
...
| **Classificação de automação** | **D → C** (fonte pública mensal confirmada e lida diretamente; mesma pendência de teste técnico de extração de texto do indicador de reajustes, acima — os dois indicadores vêm do mesmo boletim). |
...
| Reajustes salariais em negociação coletiva | 🔴 LACUNA | Fonte pública confirmada e **lida diretamente** (DIEESE — "De Olho nas Negociações") | C, potencial B pendente de teste técnico |
| Pisos salariais por categoria | 🔴 LACUNA | Fonte pública confirmada e **lida diretamente** (mesmo boletim) | C, potencial B pendente de teste técnico |

**Importante**: a 2ª rodada não deu API ao Mediador (continua sem) — deu algo melhor: os dois indicadores já vêm **calculados** pelo próprio DIEESE, com metodologia documentada, sem precisar reconstruir a lógica de negócio (instrumentos multissetoriais, pisos múltiplos, exclusões) por conta própria. Próximo passo técnico: testar extração de texto real (`pdftotext`/`pypdf`) sobre a edição mais recente antes de decidir entre pipeline B (texto extraível, como Cesta Básica) ou manter E (só PDF, sem automação). Se o projeto precisar de granularidade maior que setor/região (ex. por categoria/sindicato específico), o caminho é pedido interno à equipe do DIEESE que produz o boletim (provável ligação aos sistemas SACC/SAS internos), não engenharia de scraping externo.
