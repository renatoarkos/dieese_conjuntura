...
| # | Indicador | Instituição | Tabela/série | Método de acesso | Status | Automação | Próxima ação |
|---|---|---|---|---|---|---|---|
| 28 | Valor médio dos pisos salariais por categoria | DIEESE — boletim mensal público "De Olho nas Negociações", slide "Pisos salariais" (edição 67, abr/2026), **lido diretamente** — piso médio R$ 1.846 / mediano R$ 1.719, 1º tri/2026 | `dieese.org.br/boletimnegociacao/{ano}/boletimnegociacao{n}.pdf` | Fonte pública confirmada e lida diretamente; teste técnico de extração de texto (`pdftotext`) pendente | 🟢 CONFIRMADO — fonte pública, lida diretamente (2ª rodada, 2026-09-23) | C (potencial B) | Testar extração de texto real da edição mais recente. Alerta: valores antigos citados por terceiros (R$ 1.867/R$ 1.736, boletim nº 68) seguem NÃO confirmados — não usar |
...
| 🟢 CONFIRMADO (fonte manual/pública sem tabela extraível) | 2 | INDATEND (sem fonte pública, E definitivo); ICT |
| 🟢 CONFIRMADO (fonte pública, lida diretamente, automação C/potencial B) | 4 | Greves — número/categorias/reivindicações (E, PDF-imagem); **Reajustes salariais e Pisos salariais em negociação coletiva (atualizado, 2ª rodada Mediador)** — boletim "De Olho nas Negociações" lido diretamente, teste técnico de extração de texto pendente |
...
**Marco (2ª rodada, 2026-09-23 — reinvestigação Mediador/MTE)**: Reajustes salariais e Pisos salariais deixam de ser "PDF sem tabela extraível"/"sem fonte corrente" e passam a "fonte pública lida diretamente" — o próprio DIEESE calcula e publica os dois indicadores mensalmente no boletim "De Olho nas Negociações", com metodologia documentada. Falta apenas um teste técnico (`pdftotext`) para confirmar se viram pipeline B (como Cesta Básica) ou permanecem E.
