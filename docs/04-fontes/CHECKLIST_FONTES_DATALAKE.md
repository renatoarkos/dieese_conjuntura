...
| 25 | Rendimento médio real habitual do trabalho principal | IBGE — PNAD Contínua (bruto, já deflacionado pelo IBGE) + deflacionamento adicional DIEESE | SIDRA 5440 | API de Agregados confirmada (metadados); valores não testados | 🟢 CONFIRMADO (Lote Piloto 01), com alerta de dupla deflação | B (bruto IBGE); E (deflacionamento DIEESE) | **QF04** (validação humana) — obter Nota Técnica DIEESE |
...
| # | Indicador | Instituição | Tabela/série | Método de acesso | Status | Automação | Próxima ação |
|---|---|---|---|---|---|---|---|
| 28 | Valor médio dos pisos salariais por categoria | SACC (DIEESE-MTE, descontinuado 2004-2008, confirmado fora do ar) / boletim "De Olho nas Negociações" (cobertura não confirmada) | Ver `docs/04-fontes/dieese-publicacoes.md` | Nenhum caminho A/B confirmado nesta rodada | 🟡 Parcialmente investigado — nenhuma fonte corrente confirmada | D | **QF23-QF25** seguem relevantes. Alerta: valores citados por terceiros para o boletim nº 68 NÃO confirmados na fonte primária — não usar |
...
| 33 | Distribuição de reajustes salariais em negociação coletiva | DIEESE — boletim mensal público "De Olho nas Negociações"; Mediador/MTE confirmado sem API/exportação em massa | `dieese.org.br/boletimnegociacao/{ano}/boletimnegociacao{n}.pdf` | Fonte pública confirmada (PDF baixado), mas conteúdo é imagem/binário, sem tabela extraível | 🟢 CONFIRMADO — fonte pública, sem automação | E | Nenhuma pesquisa adicional útil — formato do PDF impede A/B/C. **QF23-QF25** seguem relevantes para o arquivo estruturado interno |
...
| 🟢 CONFIRMADO (fonte manual/pública sem tabela extraível) | 4 | INDATEND (sem fonte pública, E definitivo); ICT; Greves — número/categorias/reivindicações; **Reajustes salariais em negociação coletiva (novo)** — todos boletins públicos DIEESE, PDF sem tabela extraível, E |
| 🟡 CITADO, NÃO VALIDADO | 4 | Poucos itens do bloco Macroeconomia restante |
| 🟡 Parcialmente investigado, sem fonte corrente confirmada | 1 | **Pisos salariais por categoria** — SACC descontinuado (dados até 2007-2008, sistema fora do ar), cobertura do boletim mensal não confirmada |
...
**25 scripts de piloto técnico** (`pipelines/ingestao/`), organizados em 5 blocos — ver `docs/08-decisoes-adr/0002-expansao-piloto-por-blocos.md`. Todos testados e executados com sucesso.
...
**Marco**: de 11 indicadores 🔴 LACUNA no início desta sessão, restam apenas **6 sem nenhuma fonte pública identificada** (Sindicalização, Negociação Coletiva/Reajustes/Pisos, PIB per capita, NFSP) — os outros 5 (Cesta Básica, Combustíveis, ICT, Greves ×2) ganharam fonte confirmada, 2 deles com piloto técnico funcionando de ponta a ponta.
